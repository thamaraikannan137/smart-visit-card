...
import { Customer, CustomerCreateRequest } from "../../types/customer.type";
...
interface AddCustomerV2Props {
  onBack: () => void;
  onAddCustomer: (customerData: CustomerCreateRequest) => Promise<Customer>;
}
...
const AddCustomerV2: React.FC<AddCustomerV2Props> = ({ onBack, onAddCustomer }) => {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      // Clean up data by removing empty strings and convert to CustomerCreateRequest
      ...
      await onAddCustomer(cleanData);
      onBack();
    } catch (err: any) {
      setApiError(err.message || 'Failed to add customer. Please try again.');
    }
  };
  ...
  return (
    <Layout title="Add New Customer">
      <Box className="max-w-7xl mx-auto p-6">
        <PageHeader
          title={TITLES.ADD_CUSTOMER}
          subtitle="Create a comprehensive customer profile with all contact information"
          showBackButton
          onBack={onBack}
        />
              ...
              <CardContent className="p-6">
                ...
                <Stack spacing={4} alignItems="center" className="text-center">
                  <Avatar
                    ...
                    className="w-20 h-20 bg-gradient-to-br from-blue-500 to-purple-600 text-white text-xl"
